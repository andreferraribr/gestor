...
### Visão UO (unidade orçamentária - dono do orçamento)

![](images/visao_uo.png)

### Visão UGE (unidade gestora - executor do orçamento)

![](images/visao_uge.png)
...
```{r}
 ...
 #            kable(col.names = c("data","movimento","saldo"), format.args = list(big.mark = ".", decimal.mark = ","))
```
...
p <- ggplot( saldos %>%
               ...
               mutate(across(where(is.numeric),
                             cumsum)) %>%
               select(dia_mes, credito_disponivel) %>%
               pivot_longer(cols = credito_disponivel,
                            names_to = "item", values_to = "saldo") ,
               aes(x= dia_mes, y=  round(saldo) , label= Format(saldo, fmt = myNumFmt))) +
               geom_line()+
               facet_wrap(~item, ncol = 1)+ theme(legend.position="none",axis.title.x = element_blank())+
  ylab("saldo (R$)")+scale_y_continuous(labels = label_number(prefix = "R$", big.mark = "."))
...
p <- ggplot( saldos %>%
               ...
               mutate(across(where(is.numeric),
                             cumsum)) %>%
               select(dia_mes, limite_de_saque) %>%
               pivot_longer(cols = limite_de_saque,
                            names_to = "item", values_to = "saldo"),
               aes(x= dia_mes, y= round(saldo) , colour = item)) +
               geom_line()+
               facet_wrap(~item, ncol = 1)+ theme(legend.position="none",axis.title.x = element_blank())+
  ylab("saldo (R$)")+scale_y_continuous(labels = label_currency())+
  ylim(0, NA)
...
p <- ggplot( saldos %>%
               ...
               mutate(across(where(is.numeric),
                             cumsum)) %>%
               select(dia_mes,  a_liquidar) %>%
               pivot_longer(cols = c( a_liquidar),
                            names_to = "item", values_to = "saldo"),
               aes(x= dia_mes, y= saldo , colour = item)) +
               geom_line()+
               facet_wrap(~item, ncol = 1)+ theme(legend.position="none",axis.title.x = element_blank())+
...
p <- ggplot( saldos %>%
               ...
               mutate(across(where(is.numeric),
                             cumsum)) %>%
               select(dia_mes,  a_pagar) %>%
               pivot_longer(cols = c( a_pagar),
                            names_to = "item", values_to = "saldo"),
               aes(x= dia_mes, y= saldo , colour = item)) +
               geom_line()+
               facet_wrap(~item, ncol = 1)+ theme(legend.position="none",axis.title.x = element_blank())+
...
```{r}
label_currency <- function(accuracy = NULL, scale = 1, prefix = "R$",
...
dollar_format <- function(accuracy = NULL, scale = 1, prefix = "R$",
                          suffix = "", big.mark = ".", decimal.mark = ",",
...
dollar <- function(x, accuracy = NULL, scale = 1, prefix = "R$",
                   suffix = "", big.mark = ".", decimal.mark = ",",
                   trim = TRUE, largest_with_cents = 100000,
                   negative_parens = deprecated(),
